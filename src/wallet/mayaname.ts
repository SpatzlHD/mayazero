...
export type MayaNameDepositInput = {
...
export type MayaNameDepositSupport = {
  supported: boolean
  reason?: string
...
export function getMayaNameDepositSupport(
  manager: MayaWalletManager,
  sessionId?: string,
): MayaNameDepositSupport {
  const session = resolveSession(manager, sessionId)
  if (!session) {
    return {
      supported: false,
      reason: 'Connect a wallet session to manage MAYANames.',
    }
  }
  ...
  if (session.source === 'extension') {
    return manager.canExecute('tx.send', {
      ...
      : {
          supported: false,
          reason: 'The connected extension session cannot submit MayaChain MAYAName deposits.',
          source: session.source,
          sessionId: session.id,
          address,
        }
  ...
  return canPrepare
    ...
    : {
        supported: false,
        reason: 'The active vault session cannot prepare MayaChain MAYAName deposits.',
        source: session.source,
        sessionId: session.id,
        address,
      }
...
export async function submitMayaNameDeposit(
  manager: MayaWalletManager,
  input: MayaNameDepositInput,
): Promise<MayaNameDepositResult> {
  if (!/^\d+$/.test(input.amountBaseUnits)) {
    throw new Error('Enter a valid CACAO amount to manage this MAYAName.')
  }
  ...
  if (!memo) {
    throw new Error('A MAYAName memo is required.')
  }
  ...
  if (
    ...
    throw new WalletCapabilityError(
      'tx.prepare.send',
      session.id,
      'MayaChain MAYAName deposit preparation is unavailable for this vault session',
    )
  }
