...
export default function Header() {
  ...
  return (
    <header className="glass-panel sticky top-4 z-50 mx-4 sm:mx-6 md:mx-auto max-w-[1400px] mb-8 border border-[var(--line)] lg:border-b-0 rounded-[1.25rem] lg:rounded-b-none mt-4 transition-all">
      <nav className="flex items-center px-4 md:px-6 py-4 justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <div className="bg-[var(--line)] p-2 rounded-xl border border-[var(--inset-glint)] group-hover:bg-[var(--halo-glow)] transition-colors">
              <img src={mayaLogo} alt="Maya Logo" className="w-6 h-6 object-contain" />
            </div>
            <div className="flex flex-col">
              <span className="font-bold text-lg leading-none tracking-tight">Maya Zero</span>
              <span className="kicker hidden sm:block">Workstation</span>
            </div>
          </Link>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-8 font-semibold text-sm">
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="text-[var(--sea-ink-soft)] hover:text-[var(--cacao-neon)] transition-colors"
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          >
            {isPowerUser ? <Zap size={14} className="fill-current" /> : <Sparkles size={14} />}
            <span className="hidden md:inline">Pro Mode</span>
          </button>
          ...
          <button 
            className="lg:hidden p-2 text-[var(--sea-ink)] hover:text-[var(--maya-teal)] hover:bg-[var(--surface-strong)] rounded-xl ml-1 transition-colors"
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 mt-2 mx-0 bg-[var(--bg-base)]/95 backdrop-blur-2xl border border-[var(--line)] rounded-[1.25rem] shadow-2xl p-4 flex flex-col gap-2 animate-in slide-in-from-top-2 fade-in duration-200">
           <div className="flex flex-col gap-1 pb-4 border-b border-[var(--line)]">
             {navLinks.map((link) => (
                <Link
                  ...
                  activeProps={{ className: 'text-[var(--cacao-neon)] bg-[rgba(232,122,78,0.05)] shadow-inner' }}
                >
                  {link.label}
                </Link>
             ))}
           ...
           <div className="flex items-center justify-between p-4 sm:hidden">
             <span className="font-bold text-[var(--sea-ink)] text-sm flex items-center gap-2">Theme & Display</span>
             <ThemeToggle />
           </div>
           
           <div className="flex justify-between items-center p-4 pt-1 sm:hidden">
              <span className="font-bold text-[var(--sea-ink)] text-sm flex items-center gap-2">Pro Mode Toggle</span>
              <button
                onClick={togglePowerUser}
