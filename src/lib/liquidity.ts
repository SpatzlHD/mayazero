...
function defaultFetchMissing(): never {
  throw new Error('No fetch implementation is available for liquidity requests.')
}
  ...
  return pools
    .map((pool) => {
      ...
      const cacaoDepthBase = Number(
        formatBaseUnits(pool.cacaoDepth ?? pool.runeDepth ?? '0', MIDGARD_BASE_DECIMALS),
      )
      const cacaoUsdPrice =
        assetPriceUsd > 0 && assetPrice > 0 ? assetPriceUsd / assetPrice : 0
      const depthUsd = assetDepthBase * assetPriceUsd + cacaoDepthBase * cacaoUsdPrice
...
export function normalizeLiquidityPositions(
  ...
  const normalizedAddresses = matchingAddresses.map((address) => address.toLowerCase())
  ...
  return pools
    ...
    .sort((left, right) => {
      const leftWeight = left.state === 'active' ? 2 : left.state === 'pending' ? 1 : 0
      const rightWeight = right.state === 'active' ? 2 : right.state === 'pending' ? 1 : 0
  ...
  return actions
    ...
    .map((action) => {
      ...
      return {
        ...
        txHash: tx?.txID ?? null,
        type: action.type.toLowerCase().includes('add') ? 'deposit' : 'withdraw',
...
export function mergeLiquidityPositionsWithFallback(
  ...
  const matchIndex = positions.findIndex((position) => position.pool === fallback.pool)
  if (matchIndex === -1) {
  ...
  merged[matchIndex] = {
    ...
    matchingAddresses:
      merged[matchIndex]?.matchingAddresses.length
...
function extractBasisPoints(metadata: Record<string, unknown> | undefined): string | null {
  if (!metadata) {
    return null
  }
...
function extractMemo(metadata: Record<string, unknown> | undefined): string | null {
  if (!metadata) {
    return null
  }
