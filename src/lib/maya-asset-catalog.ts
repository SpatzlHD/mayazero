...
function defaultFetchMissing(): never {
  throw new Error(
    "No fetch implementation is available for Maya catalog loading.",
  );
}
...
async function loadMayaAssetCatalog(params: {
  ...
  const fallbackPrices = await params
    .fallbackPriceFetcher(fallbackPriceIds)
    .catch(() => ({}));
  ...
  for (const definition of Object.values(mayaChainDefinitions)) {
    ...
    if (definition.nativeAsset) {
      upsertAsset(
        ...
        createNativeAsset(
          definition,
          fallbackPrices[definition.nativeAsset.priceId ?? ""] ?? 0,
        ),
      );
    }
  }
...
function createNativeAsset(
  ...
  if (!nativeAsset) {
    throw new Error(
      `Chain ${definition.name} is missing native asset metadata`,
    );
  }
...
function createPoolAsset(pool: MayaPoolRecord): MayaSupportedAsset | null {
  ...
  const tokenId = tokenIdRaw
    ? normalizeTokenId(tokenIdRaw, definition.family)
    : undefined;
...
function upsertAsset(
  ...
  const index = chain.assets.findIndex(
...
function compareChains(
  left: MayaSupportedChain,
  right: MayaSupportedChain,
): number {
  const leftIndex = chainSortOrder.indexOf(
    left.key as (typeof chainSortOrder)[number],
  );
  const rightIndex = chainSortOrder.indexOf(
    right.key as (typeof chainSortOrder)[number],
  );
...
function compareAssets(
  left: MayaSupportedAsset,
  right: MayaSupportedAsset,
): number {
