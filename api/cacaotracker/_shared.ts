...
} from '../../src/lib/cacaotracker-types'
...
function resolveFetchImplementation(customFetch?: FetchLike): FetchLike {
  ...
  throw new Error('No fetch implementation is available for CacaoTracker requests.')
}
...
function normalizeTokenRewardsResponse(payload: unknown): MayaTokenRewardsResponse {
  const rewards = isRecord(payload) && Array.isArray(payload.rewards)
    ? payload.rewards
        .filter(isRecord)
        .map((entry) => ({
          amount: typeof entry.amount === 'string' ? entry.amount : '0',
          block_height:
            typeof entry.block_height === 'number' && Number.isFinite(entry.block_height)
              ? entry.block_height
              : 0,
          block_time: typeof entry.block_time === 'string' ? entry.block_time : '',
        }))
    : []

  return {
    address: isRecord(payload) && typeof payload.address === 'string' ? payload.address : '',
    total_cacao:
...
export async function fetchWalletSummary(
  ...
  const [rewards, syncStatus] = await Promise.all([
    fetchCacaotrackerJson<AddressRewardsResponse>(`/rewards/${encodedAddress}`, {
      ...dependencies,
      cacheBucket: 'wallet',
    }),
...
export async function fetchLiquiditySummary(
  ...
  const encodedAddress = encodeURIComponent(address)
  const [ilSummary, rewardsByPool] = await Promise.all([
    fetchCacaotrackerJson<ILSummaryAggregated>(
      `/il/${encodedAddress}/summary`,
      {
        ...dependencies,
        cacheBucket: 'liquidity',
      },
    ),
    fetchCacaotrackerJson<PoolReward[]>(
  ...
  return { ilSummary, rewardsByPool }
}
...
export async function fetchLiquidityPoolDetail(
  ...
  const [
    ...
    fetchCacaotrackerJson<PoolAnalytics>(`/pool-analytics/${encodedPool}`, {
      ...dependencies,
      cacheBucket: 'liquidity',
    }),
    fetchCacaotrackerJson<PoolMetrics[]>(
    ...
    fetchCacaotrackerJson<PoolTVLHistory[]>(
    ...
    fetchCacaotrackerJson<PoolVolumeHourly[]>(
    ...
    fetchCacaotrackerJson<PoolComparisonResult[]>('/pool-analytics/compare', {
      ...dependencies,
      cacheBucket: 'liquidity',
      query: { days: 30 },
    }).catch(() => []),
    fetchCacaotrackerJson<ILAnalysis>(`/il/${encodedAddress}/${encodedPool}`, {
      ...dependencies,
      cacheBucket: 'liquidity',
    }).catch(() => null),
    fetchCacaotrackerJson<ILHistoryResponse>(
...
export async function fetchCacaoPoolDetail(
  ...
  const encodedAddress = encodeURIComponent(address)
  const [apy, historyPayload, statsPayload, tokenRewardsPayload] = await Promise.all([
    ...
    fetchCacaotrackerJson<unknown>(
      `/cacao/history/${encodedAddress}`,
      {
        ...dependencies,
        cacheBucket: 'cacaoPool',
      },
...
export function fetchProtocolDashboard(
  ...
  return fetchCacaotrackerJson<ProtocolDashboardResponse>('/protocol/dashboard', {
    ...dependencies,
