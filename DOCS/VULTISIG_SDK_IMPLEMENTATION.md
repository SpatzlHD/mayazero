...
// Read vault file
const vultContent = fs.readFileSync(
  "MyWallet-local-party-1-share1of2.vult",
  "utf-8",
);
...
```typescript
const { results, usePhantomSolanaPath } =
  await sdk.discoverChainsFromSeedphrase(
    ...
    (progress) => {
      ...
      console.log(
        `Progress: ${progress.chainsProcessed}/${progress.chainsTotal}`,
      );
      console.log(
        `Found balances on: ${progress.chainsWithBalance.join(", ")}`,
      );
    },
  );
...
for (const [chain, balance] of Object.entries(balances)) {
  console.log(
    `${chain}: ${balance.amount} ${balance.symbol} ($${balance.fiatValue})`,
  );
}
...
try {
  ...
  if (
    ...
    error.code === VaultErrorCode.NetworkError
  ) {
...
class Vultisig {
  ...
  resendVaultVerification(options: {
    vaultId: string;
    email: string;
    password: string;
  }): Promise<void>;
...
class VaultBase {
  ...
  ): Promise<TransactionValidationResult | null>;
  simulateTransaction(
    payload: KeysignPayload,
  ): Promise<TransactionSimulationResult | null>;
...
**IndexedDB Storage**: For persistent storage, use IndexedDB (see [examples/browser](https://github.com/vultisig/docs/blob/main/developer-docs/examples/browser/README.md) for implementation).
...
{
  "files": ["dist/**/*", "node_modules/@vultisig/sdk/dist/**/*.wasm"]
}
```
...
- **Examples**:
  - [Browser Example](https://github.com/vultisig/docs/blob/main/developer-docs/examples/browser/README.md) - Full React app with UI
  - [CLI](https://github.com/vultisig/docs/blob/main/developer-docs/clients/cli/README.md) - Command-line wallet with interactive shell mode
